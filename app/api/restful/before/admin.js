...
module.exports = {
  async post(ctx, { params, params: { account, password }, role }) {
    ...
    // 校验账号是否唯一
    const mamageInfo = await getItem('Admin', { account })
  ...
  async put(ctx, { params, params: { account }, role, id }) {
    ...
    // 校验账号是否唯一
    const adminInfo = await getItem('Admin', id)
    if (!adminInfo) ctx.throw(404, '您要更新信息的超级管理员账号不存在')
    if (adminInfo.account !== account) ctx.throw(400, '超级管理员账号不允许修改')
  ...
  async del(ctx, { params, role, token, id }) {
    // 校验是否是最后一个超管账号
    const adminList = await getList('Admin')
    if (adminList.count <= 1) ctx.throw(400, '系统至少需要一个超级管理员账号')
    // 校验token是否存在
    ...
    // 校验是否是删除自己
    const userInfo = await getItem('Admin', id)
